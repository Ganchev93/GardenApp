...
**Tech Stack:** React 19, Vite, Tailwind CSS v4, Firebase v10 (Auth + Firestore + Functions + FCM), Cloudinary, Gemini 2.0 Flash, browser-image-compression, Vitest, React Testing Library

## Global Constraints

- Node.js ≥ 20 за Firebase Functions
- Firebase SDK v10 (modular — `import { getAuth } from 'firebase/auth'`, не legacy `firebase.auth()`)
...
export function useGarden(uid) {
  ...
  async function addPlant(plant) {
    ...
    await addDoc(collection(db, 'users', uid, 'garden'), {
      ...
      nextFertilizing: Timestamp.fromDate(addDays(today, plant.frequency_days)),
      photos: [],
      note: '',
      createdAt: serverTimestamp(),
    })
  ...
  async function markFertilized(id, frequency_days) {
    const today = new Date()
    await updateDoc(doc(db, 'users', uid, 'garden', id), {
      lastFertilized: Timestamp.fromDate(today),
      nextFertilizing: Timestamp.fromDate(addDays(today, frequency_days)),
    })
  }
  ...
  async function removePlant(id) {
    await deleteDoc(doc(db, 'users', uid, 'garden', id))
  }
